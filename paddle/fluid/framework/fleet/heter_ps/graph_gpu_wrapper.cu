...
void GraphGpuWrapper::init_conf(const std::string &first_node_type,
  ...
  static std::mutex mutex;
  {
    ...
    if (edge_label != "") {
      edge_label_conf_.assign(id_to_feature.size() * id_to_feature.size(), 0);
      auto items = paddle::string::split_string<std::string>(edge_label, ",");
      VLOG(2) << "edge_label[" << edge_label << "] id_to_feature.size() = " << id_to_feature.size();
      for (auto &item : items) {
        ...
        VLOG(2) << "edge_label_conf[" << src_iter->second << "][" << dst_iter->second << "] = " << label;
        edge_label_conf_[src_iter->second * id_to_feature.size() + dst_iter->second] = label;
        edge_label_conf_[dst_iter->second * id_to_feature.size() + src_iter->second] = label;
      }
      for (int i = 0; i < id_to_feature.size(); ++i) {
        std::stringstream buf;
        for (int j = 0; j < id_to_feature.size(); ++j) {
          buf << (int)edge_label_conf_[i * id_to_feature.size() + j] << " ";
        }
        VLOG(2) << "edge_label_conf[" << i << "]: " << buf.str();
      }
    }
