      ...
      while (ins_pair_flag) {
        ...
        if (FLAGS_gpugraph_storage_mode != GpuGraphStorageMode::WHOLE_HBM) {
          uint64_t *final_sage_nodes_ptr =
              reinterpret_cast<uint64_t *>(final_sage_nodes->ptr());
          if (InsertTable(
                  final_sage_nodes_ptr, uniq_instance, d_uniq_node_num_) != 0) {
            VLOG(2)
                << "in train sage mode, insert uniq nodes stage, table is full";
            break;
          }
        }
      ...
      while (total_instance != 0) {
        ...
        if (FLAGS_gpugraph_storage_mode != GpuGraphStorageMode::WHOLE_HBM) {
          ...
          if (InsertTable(
                  final_sage_nodes_ptr, uniq_instance, d_uniq_node_num_) != 0) {
            VLOG(2)
                << "in infer sage mode, insert uniq nodes stage, table is full";
            break;
          }
        }
        final_sage_nodes_vec_.emplace_back(final_sage_nodes);
...
void GraphDataGenerator::AllocTrainResource(int thread_id) {
  if (slot_num_ > 0) {
    ...
    cudaMemcpy(d_slot_feature_num_map_->ptr(),
               ...
               cudaMemcpyHostToDevice);
    d_actual_slot_id_map_ =
        memory::Alloc(place_, fea_num_per_node_ * sizeof(int));
