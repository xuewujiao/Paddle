...
__global__ void fill_shuffled_meta_row(int *original_rows,
                                       int *shuffled_row,
                                       int *shuffled_meta_row,
                                       int len) {
  const size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < len) {
    shuffled_meta_row[i] = original_rows[shuffled_row[i]];
  }
}
...
__global__ void GraphFillIdKernelWithType(uint64_t *id_tensor,
                                          ...
                                          int node_type_size) {
  __shared__ uint64_t local_key[CUDA_NUM_THREADS * 2];
  __shared__ int local_type[CUDA_NUM_THREADS * 2];
  __shared__ int local_num;
  __shared__ int global_num;
  ...
  if (idx < len) {
    int src = row[idx] * col_num + central_word;
    int type_src = row_meta_path[idx] * col_num + central_word;
    if (walk[src] != 0 && walk[src + step] != 0) {
      size_t dst = atomicAdd(&local_num, 1);
      local_key[dst * 2] = walk[src];
      local_key[dst * 2 + 1] = walk[src + step];
      if (type_src >= row_num * col_num) {
        printf(
            "error type out of bound %d %d %d\n", type_src, row_num, col_num);
      }
      if (type_src + step >= row_num * col_num) {
        printf("error type out of bound %d %d %d %d\n",
               type_src,
               step,
               row_num,
               col_num);
      }
      local_type[dst * 2] = meta_path_types[type_src];
      local_type[dst * 2 + 1] = meta_path_types[type_src + step];
      if (local_type[dst * 2] >= node_type_size || local_type[dst * 2] < 0) {
        printf("generated node type out of bound %d\n", local_type[dst * 2]);
      }
      if (local_type[dst * 2 + 1] >= node_type_size ||
          local_type[dst * 2 + 1] < 0) {
        printf("generated node type out of bound %d\n",
               local_type[dst * 2 + 1]);
      }
    }
  ...
  if (threadIdx.x < local_num) {
    ...
    id_tensor[global_num * 2 + 2 * threadIdx.x + 1] =
        local_key[2 * threadIdx.x + 1];
    id_types[global_num * 2 + 2 * threadIdx.x] = local_type[2 * threadIdx.x];
    id_types[global_num * 2 + 2 * threadIdx.x + 1] =
        local_type[2 * threadIdx.x + 1];
  }
}
...
int GraphDataGenerator::FillInsBuf() {
  ...
  int *d_ins_type_buf, *d_shuffled_meta_row, *d_meta_path_node_types;
  if (type_optimization) {
    d_ins_type_buf = reinterpret_cast<int *>(d_ins_type_buf_->ptr());
    d_shuffled_meta_row =
        reinterpret_cast<int *>(d_shuffled_row_meta_path_->ptr());
    d_meta_path_node_types =
        reinterpret_cast<int *>(d_meta_path_node_types_->ptr());
  }
  cudaMemsetAsync(d_pair_num, 0, sizeof(int), stream_);
  int len = buf_state_.len;

  if (type_optimization) {
    GraphFillIdKernelWithType<<<GET_BLOCKS(len),
                                CUDA_NUM_THREADS,
                                0,
                                stream_>>>(
        ins_buf + ins_buf_pair_len_ * 2,
        d_ins_type_buf + ins_buf_pair_len_ * 2,
        d_shuffled_meta_row + buf_state_.cursor,
        d_meta_path_node_types,
        d_pair_num,
        walk,
        ...
        walk_len_,
        meta_path_.size(),
        (int)GraphGpuWrapper::GetInstance()->feature_to_id.size());
  } else {
...
GraphDataGenerator::GetReindexResultWithTypeInfo(int64_t *reindex_src_data,
  ...
  auto all_types =
      memory::AllocShared(place_, (node_len + neighbor_len) * sizeof(int));
  int *all_types_data = reinterpret_cast<int *>(all_types->ptr());

  VLOG(2) << gpuid_ << ": cudaMemcpy all_nodes_data";
  cudaMemcpy(all_nodes_data,
             center_nodes,
             sizeof(int64_t) * node_len,
             cudaMemcpyDeviceToDevice);
  cudaMemcpy(all_nodes_data + node_len,
             reindex_src_data,
  ...
  phi::ReindexSrcOutput<int64_t>
      <<<GET_BLOCKS(neighbor_len), CUDA_NUM_THREADS, 0, stream_>>>(
          reindex_src_data,
          neighbor_len,
          reindex_table_size_,
          d_reindex_table_key_ptr,
...
std::shared_ptr<phi::Allocation> GraphDataGenerator::GetReindexResult(
  ...
  VLOG(2) << gpuid_ << ": Run phi::FillHashTable";
  auto final_nodes =
      phi::FillHashTable<int64_t, phi::GPUContext>(dev_ctx_,
                                                   all_nodes_data,
                                                   node_len + neighbor_len,
                                                   reindex_table_size_,
                                                   d_reindex_table_key_ptr,
...
__global__ void varifyNodeType(
    int *node_type, int len, int node_type_num, int *res, int index) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < len) {
    if (node_type[idx] >= node_type_num || node_type[idx] < 0) {
      if (res[0] < 1000) {
        printf("finds an invalid node_type %d %d %d\n",
               idx,
               node_type[idx],
               index);
      }
...
GraphDataGenerator::GenerateSampleGraphWithTypeInfo(uint64_t *node_ids,
  ...
  cudaMemcpy(in_x.data<int64_t>(),
             node_ids,
             len * sizeof(uint64_t),
             cudaMemcpyDeviceToDevice);
  auto gpu_graph_ptr = GraphGpuWrapper::GetInstance();

  phi::DenseTensor uniq_nodes, index, count;
  std::vector<int> axis;
  phi::UniqueKernel<int64_t, phi::GPUContext>(dev_ctx_,
                                              in_x,
                                              true,
                                              true,
                                              false,
  ...
  auto unique_node_types = memory::AllocShared(place_, uniq_len * sizeof(int));
  int node_type_size = (int)gpu_graph_ptr->feature_to_id.size();
  int *unique_node_types_ptr =
      reinterpret_cast<int *>(unique_node_types->ptr());
  // {
  //     auto type_node_not_valid = memory::Alloc(place_, sizeof(int));

  //     cudaMemsetAsync(reinterpret_cast<int*>(type_node_not_valid->ptr()), 0,
  //     sizeof(int), stream_); varifyNodeType<<<GET_BLOCKS(uniq_len),
  //                            CUDA_NUM_THREADS,0,stream_>>>(node_types,
  //                            len,node_type_size,reinterpret_cast<int*>(type_node_not_valid->ptr()),0);
  //     int not_valid = 0;
  //       cudaMemcpyAsync(
  //       &not_valid,
  //       reinterpret_cast<int*>(type_node_not_valid->ptr()),
  //       sizeof(int),
  //       cudaMemcpyDeviceToHost,
  //       stream_);
  //     cudaStreamSynchronize(stream_);
  ...
  //             "node type array contains element out of range"));
  //     PADDLE_ENFORCE_GT(
  //         len,
  //         0,
  //         platform::errors::PreconditionNotMet(
  //             "len %d is not valid.", len));
  // }
  // cudaStreamSynchronize(stream_);
  fill_shuffled_meta_row<<<GET_BLOCKS(uniq_len),
                           CUDA_NUM_THREADS,
                           0,
                           stream_>>>(
      node_types, index_data, unique_node_types_ptr, uniq_len);
  // {
  //     auto type_node_not_valid = memory::Alloc(place_, sizeof(int));
  ...
  //     sizeof(int), stream_); varifyNodeType<<<GET_BLOCKS(uniq_len),
  //                            CUDA_NUM_THREADS,0,stream_>>>(node_types,
  //                            len,node_type_size,reinterpret_cast<int*>(type_node_not_valid->ptr()),1);
  //     int not_valid = 0;
  //       cudaMemcpyAsync(
  //       &not_valid,
  //       reinterpret_cast<int*>(type_node_not_valid->ptr()),
  ...
  for (int i = 0; i < len_samples; i++) {
    ...
    std::shared_ptr<phi::Allocation> neighbors, reindex_dst, type_info;
    int neighbors_len = 0;
    if (i == 0) {
      VLOG(2) << "Sample first layer";
      neighbors_len = 0;
      cudaStreamSynchronize(stream_);
    ...
    if (i == 0) {
      auto res = GetReindexResultWithTypeInfo(
          reindex_src_data,
          uniq_nodes_data,
    ...
    // if(neighbors_len == 0){
    //   VLOG(0)<<"error --> doesnt' sample any neighbors "<<gpuid_<<" len =
    //   "<<uniq_len; display_device_array<uint64_t>((uint64_t
    //   *)uniq_nodes_data,min(uniq_len,200),"uniq_nodes_data");
    //   display_device_array<int>(unique_node_types_ptr,min(uniq_len,200),"unique_node_types_ptr");
    // }

    cudaMemcpyAsync(num_nodes_tensor_ptr_[i],
                    final_nodes_len_vec.data() + i,
                    sizeof(int),
...
int GraphDataGenerator::GenerateBatch() {
  ...
  if (!gpu_graph_training_) {
    while (cursor_ < h_device_keys_.size()) {
      ...
      int node_type = key_type[cursor_];
      ...
      total_instance *= 2;
      if (!sage_mode_) {
        id_tensor_ptr_ = feed_vec_[0]->mutable_data<int64_t>(
        ...
        auto node_buf =
            memory::AllocShared(place_, total_instance * sizeof(uint64_t));
        ...
        std::shared_ptr<phi::Allocation> d_ins_type_buf;
        int *node_types;
        // if (type_optimization) {
        //   d_ins_type_buf =
        //       memory::AllocShared(place_, total_instance * sizeof(int));
        //   node_types = reinterpret_cast<int *>(d_ins_type_buf->ptr());
        //   thrust::device_ptr<int> dev_node_type_ptr =
        //       thrust::device_pointer_cast(node_types);

        //   const auto &exec_policy = thrust::cuda::par.on(stream_);
        //   thrust::fill(exec_policy,
        //                dev_node_type_ptr,
        //                dev_node_type_ptr + total_instance,
        //                (int)node_type);
        //   VLOG(2) << "infer GenerateSampleGraphWithTypeInfo";
        //   final_sage_nodes = GenerateSampleGraphWithTypeInfo(node_buf_ptr_,
        //                                                      node_types,
        //                                                      total_instance,
        //                                                      &uniq_instance_,
        //                                                      &inverse_);
        //   VLOG(2) << "infer GenerateSampleGraphWithTypeInfo done";
        //   VLOG(2) << "uniq_nodes-number " << uniq_instance_;
        // } else {
        final_sage_nodes = GenerateSampleGraph(
            node_buf_ptr_, total_instance, &uniq_instance_, &inverse_);
        // }
        id_tensor_ptr_ = feed_vec_[0]->mutable_data<int64_t>(
            {uniq_instance_, 1}, this->place_);
        show_tensor_ptr_ =
        ...
        cudaMemcpy(id_tensor_ptr_,
                   final_sage_nodes->ptr(),
                   sizeof(int64_t) * uniq_instance_,
                   cudaMemcpyDeviceToDevice);
        cudaMemcpy(index_tensor_ptr_,
                   inverse_.data<int>(),
  ...
  int *ins_type_cursor, *ins_type_buf;
  phi::DenseTensor inverse;
  if (gpu_graph_training_) {
    ...
    if (type_optimization) {
      ins_type_buf = reinterpret_cast<int *>(d_ins_type_buf_->ptr());
      ins_type_cursor = ins_type_buf + ins_buf_pair_len_ * 2 - total_instance;
    }
    if (!sage_mode_) {
      ...
      VLOG(2) << gpuid_ << " "
              << "Ready to enter GenerateSampleGraph";
      if (type_optimization) {
        final_sage_nodes = GenerateSampleGraphWithTypeInfo(ins_cursor,
                                                           ins_type_cursor,
                                                           total_instance,
                                                           &uniq_instance_,
                                                           &inverse);
        ...
        final_sage_nodes = GenerateSampleGraph(
            ins_cursor, total_instance, &uniq_instance_, &inverse);
      }
      VLOG(2) << "Copy Final Results";
      id_tensor_ptr_ = feed_vec_[0]->mutable_data<int64_t>({uniq_instance_, 1},
                                                           this->place_);
      show_tensor_ptr_ =
          feed_vec_[1]->mutable_data<int64_t>({uniq_instance_}, this->place_);
      clk_tensor_ptr_ =
      ...
      VLOG(2) << "copy id_tensor";
      cudaMemcpyAsync(id_tensor_ptr_,
                      final_sage_nodes->ptr(),
                      sizeof(int64_t) * uniq_instance_,
                      cudaMemcpyDeviceToDevice,
                      stream_);
      VLOG(2) << "copy id_tensor done";
      cudaMemcpyAsync(index_tensor_ptr_,
                      inverse.data<int>(),
                      sizeof(int) * total_instance,
                      cudaMemcpyDeviceToDevice,
                      stream_);
      VLOG(2) << "copy id_tensor index done";
      GraphFillCVMKernel<<<GET_BLOCKS(uniq_instance_),
                           CUDA_NUM_THREADS,
                           0,
                           stream_>>>(show_tensor_ptr_, uniq_instance_);
  ...
  if (slot_num_ > 0) {
    ...
    if (sage_mode_) {
      ...
      if (d_feature_buf_ == NULL ||
          d_feature_buf_->size() < temp_storage_bytes) {
...
int GraphDataGenerator::FillWalkBuf(std::shared_ptr<phi::Allocation> d_walk) {
  ...
  int *d_row_meta_path, *d_shuffled_row_meta_path;
  thrust::device_ptr<int> dev_row_meta_path_ptr;
  if (type_optimization) {
    VLOG(2) << "in type opti3";
    // d_sample_keys = reinterpret_cast<uint64_t *>(d_sample_keys_->ptr());
    d_row_meta_path = reinterpret_cast<int *>(d_row_meta_path_->ptr());
    VLOG(2) << "in type opti4";
    d_shuffled_row_meta_path =
        reinterpret_cast<int *>(d_shuffled_row_meta_path_->ptr());
    VLOG(2) << "make thrust array--";
    dev_row_meta_path_ptr = thrust::device_pointer_cast(d_row_meta_path);
    VLOG(2) << "make thrust array done";
  }
  ...
  while (i <= remain_size) {
    ...
    if (type_optimization) {
      VLOG(2) << "fill row_meta_path from range " << total_row << " "
              << total_row + jump_rows_;
      const auto &exec_policy = thrust::cuda::par.on(stream_);
      thrust::fill(exec_policy,
                   dev_row_meta_path_ptr + total_row,
                   dev_row_meta_path_ptr + total_row + jump_rows_,
                   (int)cur_node_idx);
      VLOG(2) << "fill row_meta_path from range " << total_row << " "
              << total_row + jump_rows_ << "done";
    }
    total_row += jump_rows_;
    cursor_ += 1;
  }
  ...
  thrust::shuffle_copy(exec_policy,
                       ...
                       engine);
  if (type_optimization) {
    VLOG(2) << "fill_shuffled_meta_row ";
    fill_shuffled_meta_row<<<GET_BLOCKS(total_row),
                             CUDA_NUM_THREADS,
                             0,
                             stream_>>>(
        d_row_meta_path, d_random_row, d_shuffled_row_meta_path, total_row);
    VLOG(2) << "fill_shuffled_meta_row done";
  }
  cudaStreamSynchronize(stream_);
...
void GraphDataGenerator::AllocResource(const paddle::platform::Place &place,
  ...
  cudaMemcpy(d_slot_feature_num_map_->ptr(),
             h_slot_feature_num_map_.data(),
             sizeof(int) * slot_num_,
             cudaMemcpyHostToDevice);
  d_actual_slot_id_map_ = memory::Alloc(place, fea_num_per_node_ * sizeof(int));
  cudaMemcpy(d_actual_slot_id_map_->ptr(),
             h_actual_slot_id_map.data(),
             sizeof(int) * fea_num_per_node_,
             cudaMemcpyHostToDevice);
  d_fea_offset_map_ = memory::Alloc(place, fea_num_per_node_ * sizeof(int));
  cudaMemcpy(d_fea_offset_map_->ptr(),
             h_fea_offset_map.data(),
             sizeof(int) * fea_num_per_node_,
             cudaMemcpyHostToDevice);
  ...
  if (type_optimization) {
    VLOG(2) << "type optimization alloc array";
    d_row_meta_path_ = memory::AllocShared(
        place_,
        walk_degree_ * once_sample_startid_len_ * repeat_time_ * sizeof(int));
    d_shuffled_row_meta_path_ = memory::AllocShared(
        place_,
        walk_degree_ * once_sample_startid_len_ * repeat_time_ * sizeof(int));
    d_meta_path_node_types_ = memory::AllocShared(
        place_, meta_path_to_node_type_.size() * walk_len_ * sizeof(int));
    VLOG(2) << "d_meta_path_node_types_ alloc size = "
            << meta_path_to_node_type_.size() * walk_len_ * sizeof(int);
  }
  ...
  if (type_optimization) {
    d_ins_type_buf_ =
        memory::AllocShared(place_, (batch_size_ * 2 * 2) * sizeof(int));
  }
  if (slot_num_ > 0) {
    if (!sage_mode_) {
  ...
  if (type_optimization) {
    int *d_meta_path_node_types =
        reinterpret_cast<int *>(d_meta_path_node_types_->ptr());
    for (int i = 0; i < meta_path_to_node_type_.size(); i++) {
      if (meta_path_to_node_type_[i].size()) {
        VLOG(2) << "in display meta_path_to_node_type " << i;
        for (int j = 0; j < meta_path_to_node_type_[i].size(); j++)
          VLOG(2) << j << " : " << meta_path_to_node_type_[i][j];
        CUDA_CHECK(
            cudaMemcpyAsync(d_meta_path_node_types + i * walk_len_,
  ...
  VLOG(2) << "graph datafeed allocation done";
  cudaStreamSynchronize(stream_);
}
...
void GraphDataGenerator::SetConfig(
  ...
  type_optimization = graph_config.type_optimization();
  ...
  meta_path_.resize(node_types.size());
  meta_path_to_node_type_.resize(node_types.size());
  ...
  for (size_t i = 0; i < meta_paths.size(); i++) {
    ...
    for (auto &node : nodes) {
      ...
      auto node_types = paddle::string::split_string<std::string>(node, "2");
      PADDLE_ENFORCE_EQ(node_types.size(),
                        2,
