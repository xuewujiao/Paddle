...
static bool check_gpu_rdma_status(const int &device_count,
                                  std::vector<int> *gpu_status) {
  // a100
  std::string str =
      excute_cmd_result("source ~/.bashrc && nvidia-smi topo -m | grep mlx5");
...
#if defined(PADDLE_WITH_CUDA)
  rdma_trans_ = check_gpu_rdma_status(dev_ids_.size(), rdma_status_);
#endif
}
