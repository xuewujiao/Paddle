...
class HeterComm {
 ...
 protected:
  ...
  void pull_sparse_all2all(const int gpu_id,
                           ...
                           float* d_vals,
                           size_t len);
  void shard_inner_keys(const int& total_fea_num,
                        const KeyType* d_keys,
                        const int& gpu_id,
                        const int& gpu_num,
                        InnerResource* res);
  void gather_inner_keys_p2p(const int& total_fea_num,
                             const KeyType* d_keys,
                             InnerResource& res,
                             const int gpu_id,
                             const int gpu_num,
                             const int trans_id);
  int gather_inter_keys_by_copy(const int gpu_id,
  ...
  void partition_shard_keys(const int& gpu_id,
                            ...
                            size_t* h_part_sizes,
                            const int& shard_num);
  void send_data_by_all2all(const int& gpu_id,
                            ...
                            char* d_rev_buff,
                            cudaStream_t& stream);
  int gather_sparse_keys_by_all2all(const int gpu_id,
                                    const size_t& fea_size,
  ...
  void scatter_sparse_vals_by_all2all(const int gpu_id,
                                      ...
                                      void* d_out_vals);
  void scatter_inner_vals_p2p(const int total_fea_num,
                              void* d_out_vals,
                              InnerResource& res,
                              const int gpu_id,
                              const int gpu_num,
  ...
  void scatter_inter_vals_by_copy(const int gpu_id,
                                  ...
                                  const char* d_in_vals,
                                  void* d_out_vals);
  void gather_inner_data_p2p(const int total_fea_num,
                             const KeyType* d_keys,
                             const void* d_vals,
                             InnerResource& res,
                             const int gpu_id,
  ...
  void push_sparse_all2all(const int& gpu_id,
                           ...
                           size_t len,
                           Sgd& sgd);  // NOLINT
  int merge_grad(const int& gpu_id,
                 const size_t& len,
                 const KeyType* d_in_keys,
                 KeyType* d_out_keys,
                 const void* d_in_grads,
                 void* d_out_grads);
  int gather_inter_gradient_by_copy(const int& gpu_id,
                                    const int& push_size,
                                    KeyType* d_keys,
                                    void* d_push_vals);
  int gather_sparse_gradient_by_all2all(const int& gpu_id,
                                        const int& push_size,
                                        const KeyType* d_keys,
