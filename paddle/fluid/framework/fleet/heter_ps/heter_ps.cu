...
HeterPsBase* HeterPsBase::get_instance(
  ...
  if (accessor_type == "CtrDymfAccessor") {
    ...
    VLOG(0) << " HeterPsBase get_instance Warning: now only support "
               "CtrDymfAccessor, but get " << accessor_type;
  }
}
...
template <typename GPUAccessor, template<typename T> class GPUOptimizer>
void HeterPs<GPUAccessor, GPUOptimizer>::pull_sparse(int num, FeatureKey* d_keys,
...
void HeterPs<GPUAccessor, GPUOptimizer>::build_ps(int num, FeatureKey* h_keys, char* pool,
                                   size_t len, size_t feature_value_size,
...
template <typename GPUAccessor, template<typename T> class GPUOptimizer>
void HeterPs<GPUAccessor, GPUOptimizer>::push_sparse(int num, FeatureKey* d_keys, float* d_grads,
                          size_t len) {
...
template <typename GPUAccessor, template<typename T> class GPUOptimizer>
int HeterPs<GPUAccessor, GPUOptimizer>::dedup_keys_and_fillidx(const int gpu_id, 
