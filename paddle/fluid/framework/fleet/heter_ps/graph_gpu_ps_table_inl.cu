...
int GpuPsGraphTable::get_feature_info_of_nodes_all2all(
   ...
   CUDA_CHECK(cudaMemcpyAsync(inter_feature_list_ptr,
                               loc.d_merged_vals,
   ...
   send_vari_vals_by_all2all(gpu_id,
                             ...
                             reinterpret_cast<uint8_t*>(loc.d_merged_vals),
                             stream);
   ...
   CUDA_CHECK(cudaMemcpyAsync(inter_slot_list_ptr,
                                loc.d_merged_vals,
...
int GpuPsGraphTable::get_feature_info_of_nodes_normal(
  ...
  int grid_size = (node_num - 1) / block_size_ + 1;
  //allocate size list and size list predixsum
  size_list =
       memory::Alloc(place,
                     node_num * sizeof(uint32_t),
                     phi::Stream(reinterpret_cast<phi::StreamId>(stream)));
