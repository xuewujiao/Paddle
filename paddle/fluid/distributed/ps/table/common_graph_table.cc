...
void GraphTable::graph_partition(bool is_edge) {
  ...
  } else if (mode == "fennelcx" || mode == "FENNELCX") {
    VLOG(0) << "Graph partitioning FENNEL CX";
    if (is_edge) {
      fennel_graph_edge_partition_cx();
    } else {
      fennel_graph_feature_partition();
    }
    VLOG(0) << "Graph partitioning FENNEL CX Done";
  } else if (mode == "fennelbx" || mode == "FENNELBX" || mode == "fennel" ||
             mode == "FENNEL") {
    VLOG(0) << "Graph partitioning FENNEL BX";
    if (is_edge) {
      fennel_graph_edge_partition_bx();
    } else {
      fennel_graph_feature_partition();
    }
    VLOG(0) << "Graph partitioning FENNEL BX Done";
  } else if (mode == "hard" || mode == "HARD") {
    VLOG(0) << "Graph partitioning Hard Hash Split";
  } else if (mode == "hard2" || mode == "HARD2") {
    VLOG(0) << "Graph partitioning Hard2";
    if (is_edge) {
      hard_graph_edge_partition();
    } else {
      hard_graph_feature_partition();
    }
    ...
    cal_tasks.push_back(_shards_task_pool[idx % task_pool_size_]->enqueue(
        [&, idx, this]() -> std::pair<size_t, size_t> {
          ...
          for (size_t part_id = 0; part_id < shard_num_per_server; part_id++) {
            shard_tasks.push_back(load_node_edge_task_pool->enqueue(
                [&,
                 ...
                 idx,
                 //  shard_num,
                 this]() -> std::pair<size_t, size_t> {
                  auto &shards = tmp_edge_shards[idx][part_id]->get_bucket();
                  size_t cut_edge_size = 0;
                  size_t all_edge_size = 0;
                  for (auto node : shards) {
                    ...
                    for (size_t n_i = 0; n_i < node->get_neighbor_size();
                      ...
                      for (auto &search_shard : tmp_edge_shards) {
                        tmp_node =
                            search_shard[d_id % shard_num]->find_node(d_id);
                        if (tmp_node != nullptr) {
                          break;
                        }
                      }
                      ...
                      Node *node = nullptr;
                      for (auto &search_shard : edge_shards) {
                        node = search_shard[d_id % shard_num]->find_node(d_id);
                        if (node != nullptr) {
                          all_edge_size++;
                          break;
                        }
                      ...
                      if (tmp_node == nullptr && node != nullptr) {
                        cut_edge_size++;
                      }
                    }
                  }
                  return {cut_edge_size, all_edge_size};
                }));
          }
          size_t cut_size = 0;
          size_t all_edge_size = 0;
          for (size_t j = 0; j < shard_tasks.size(); j++) {
            ...
            cut_size += res.first;
            all_edge_size += res.second;
          }
          return {cut_size, all_edge_size};
        }));
  }
  size_t all_cut_size = 0;
  size_t all_edge_size = 0;
  for (size_t j = 0; j < cal_tasks.size(); j++) {
    auto res = cal_tasks[j].get();
    all_cut_size += res.first;
    all_edge_size += res.second;
  }
  VLOG(0) << "cut edge and node shards size: " << all_cut_size
          << " all_edge_size:" << all_edge_size;
...
void GraphTable::dbh_graph_feature_partition() {
  ...
  for (auto &it : this->feature_to_id) {
    ...
    for (size_t i = 0; i < shard_num_per_server; i++) {
      tasks.push_back(load_node_edge_task_pool->enqueue([&, node_idx, i, this]()
                                                            -> size_t {
        ...
        for (auto node : shards) {
          ...
          if (!is_key_for_self_rank(id) &&
              (unique_all_edge_keys_.find(id) == unique_all_edge_keys_.end())) {
            remove_keys.push_back(id);
          }
          if ((unique_all_edge_keys_.find(id) != unique_all_edge_keys_.end()) &&
              !is_key_for_self_rank(id)) {
            duplicate_keys.push_back(id);
          }
        }
        ...
        VLOG(0) << node_idx << "  " << i
                ...
                << ",remove_keys:" << remove_keys.size();
        size_t num = duplicate_keys.size();
        return num;
      }));
    }
  }
  size_t duplicate_keys_all = 0;
  for (size_t i = 0; i < tasks.size(); ++i) {
    duplicate_keys_all += tasks[i].get();
  }
  unique_all_edge_keys_.clear();
  VLOG(0) << "end to process dbh feature shard, duplicate_keys_all:"
          << duplicate_keys_all;
}

void GraphTable::fix_feature_node_shards(bool load_slot) {
  CHECK(load_slot) << "only support feature node mode";
  VLOG(0) << "begin fix feature node type count=" << feature_shards.size()
  ...
  VLOG(0) << "fix feature node count=" << total << ", add count=" << add_cnt
          << ", with slot=" << load_slot;
}

void GraphTable::hard_graph_edge_partition() {
  VLOG(0) << "start to process hard egde shard";
  std::vector<std::vector<GraphShard *>> tmp_edge_shards;
  tmp_edge_shards.resize(edge_shards.size());
  for (size_t k = 0; k < edge_shards.size(); k++) {
    for (size_t i = 0; i < shard_num_per_server; i++) {
  ...
  std::vector<std::future<int>> tasks;
  std::vector<std::mutex *> node_mutex(node_num_);
  for (int i = 0; i < node_num_; i++) {
    node_mutex[i] = new std::mutex();
  }
  for (size_t idx = 0; idx < id_to_edge.size(); idx++) {
    tasks.push_back(_shards_task_pool[idx % task_pool_size_]->enqueue(
        [&, idx, node_mutex, this]() -> int {
          std::vector<std::future<int>> shard_tasks;
          for (size_t part_id = 0; part_id < shard_num; ++part_id) {
            shard_tasks.push_back(load_node_edge_task_pool->enqueue(
                [&, part_id, idx, this]() -> int {
                  auto &shards = edge_shards[idx][part_id]->get_bucket();
                  for (auto node : shards) {
                    uint64_t id = node->get_id();
                    // 由于节点id的度计算需要到不同类型的边表中去查找
                    if (is_key_for_self_rank(id)) {
                      tmp_edge_shards[idx][part_id]->add_graph_node(node);
                    }
                    int rank = partition_for_rank(id);
                    node_mutex[rank]->lock();
                    egde_node_ids_[rank].insert(id);
                    node_mutex[rank]->unlock();
                  }
                  return 0;
                }));
          }
...
void GraphTable::fennel_graph_edge_partition_cx() {
  ...
  int gnode_counts = all_edge_keys[0].size();
  const double gamma = 3 / 2.0;
  const double alpha =
      edge_counts_ * pow(node_num_, (gamma - 1)) / pow(gnode_counts, gamma);
  const double load_limit = 1.1 * gnode_counts / node_num_;
  VLOG(1) << " gnode_counts:" << gnode_counts
          << " edge_counts_:" << edge_counts_ << " alpha:" << alpha
  ...
  srand(500);
  for (size_t k = 0; k < edge_shards.size(); k++) {
    auto &shard = edge_shards[k][0]->get_bucket();
    for (int i = 0; i < node_num_; i++) {
      int rand_id = rand() % shard.size() + node_num_ - i;
      auto id = shard[rand_id]->get_id();
      egde_node_ids_[i].insert(id);
      if (i == node_id_) {
        tmp_edge_shards[k][0]->add_graph_node(shard[rand_id]);
      }
      VLOG(3) << k << " " << i << " add " << rand_id << " id:" << id;
    }
  }

  // all edges
  // 记录每台机器已存的节点
  std::vector<size_t> sub_node_sizes(node_num_, edge_shards.size());
  for (size_t idx = 0; idx < edge_shards.size(); idx++) {
    for (size_t part_id = 0; part_id < shard_num_per_server; part_id++) {
      auto &shards = edge_shards[idx][part_id]->get_bucket();
      for (auto node : shards) {
        std::vector<double> score(node_num_, 0);
        auto nid = node->get_id();
        ...
        for (int i = 0; i < node_num_; i++) {
          ...
          if (node_size <= load_limit) {
            ...
            score[i] = inter_cost - alpha * gamma * pow(node_size, gamma - 1);
          }
        }
        // 计算最大score所在的机器
        double max_score = INT_MIN;
        int index = 0;
        for (int s_id = 0; s_id < node_num_; s_id++) {
          if (score[s_id] > max_score) {
            max_score = score[s_id];
            index = s_id;
          }
        }
        // 节点插入到对应的机器shard中
  ...
  for (size_t idx = 0; idx < edge_shards.size(); idx++) {
    cal_tasks.push_back(_shards_task_pool[idx % task_pool_size_]->enqueue(
        [&, idx, this]() -> std::pair<size_t, size_t> {
          ...
          for (size_t part_id = 0; part_id < shard_num_per_server; part_id++) {
            shard_tasks.push_back(load_node_edge_task_pool->enqueue(
                [&, part_id, idx, this]() -> std::pair<size_t, size_t> {
                  auto &shards = tmp_edge_shards[idx][part_id]->get_bucket();
                  size_t cut_edge_size = 0;
                  size_t all_edge_size = 0;
                  for (auto node : shards) {
  ...
  // clear_edge_shard();
  edge_shards = std::move(tmp_edge_shards);
  std::vector<std::vector<uint64_t>> new_all_edge_keys;
  unique_all_edge_keys_.clear();
  VLOG(1) << "begin to get NEW all_edge_keys";
  this->get_all_id(GraphTableType::EDGE_TABLE, 1, &new_all_edge_keys);
  VLOG(1) << "end to get NEW all_edge_keys, size:"
          << new_all_edge_keys[0].size();
  unique_all_edge_keys_.insert(new_all_edge_keys[0].begin(),
                               new_all_edge_keys[0].end());
  VLOG(1) << "insert unique_all_edge_keys_ done, size:"
          << unique_all_edge_keys_.size();
  for (int i = 0; i < node_num_; i++) {
  ...
  VLOG(0) << "end to process fennel cx egde shard";
}

// 并行
void GraphTable::fennel_graph_edge_partition_bx() {
  VLOG(0) << "start to process fennel egde shard";
  std::vector<std::vector<uint64_t>> all_edge_keys;
  unique_all_edge_keys_.clear();
  VLOG(1) << "begin to get all_edge_keys";
  ...
  const double alpha =
      edge_counts_ * pow(node_num_, (gamma - 1)) / pow(gnode_counts, gamma);
  const double load_limit = 1.1 * gnode_counts / node_num_;
  auto edge_counts = edge_counts_.load();
  VLOG(0) << " gnode_counts:" << gnode_counts << " edge_counts_:" << edge_counts
          << " alpha:" << alpha << " load_limit:" << load_limit;
  ...
  for (size_t idx = 0; idx < edge_shards.size(); idx++) {
    tasks.push_back(_shards_task_pool[idx % task_pool_size_]->enqueue(
        [&, idx, node_mutex, this]() -> size_t {
          std::vector<std::future<size_t>> shard_tasks;
          for (size_t part_id = 0; part_id < shard_num_per_server; part_id++) {
            shard_tasks.push_back(load_node_edge_task_pool->enqueue(
                [&, part_id, idx, node_mutex, this]() -> size_t {
                  auto &shards = edge_shards[idx][part_id]->get_bucket();
                  for (auto node : shards) {
                    std::vector<double> score(node_num_, 0);
                    auto nid = node->get_id();
                    ...
                    for (int i = 0; i < node_num_; i++) {
                      ...
                      if (node_size <= load_limit) {
                        ...
                        for (size_t n_i = 0; n_i < node->get_neighbor_size();
                             ++n_i) {
                          auto d_id = node->get_neighbor_id(n_i);
                          if (egde_node_ids_[i].find(d_id) !=
                              egde_node_ids_[i].end()) {
                            inter_cost++;
                          }
                        }
                        score[i] = inter_cost -
                                   alpha * gamma * pow(node_size, gamma - 1);
                      }
                    }
                    // 计算最大score所在的机器
                    double max_score = INT_MIN;
                    ...
                    node_mutex[index]->lock();
                    sub_node_sizes[index]++;
                    egde_node_ids_[index].insert(nid);
                    node_mutex[index]->unlock();
                  }
                  return 0;
                }));
          }
  ...
  for (size_t i = 0; i < node_mutex.size(); ++i) {
    delete node_mutex[i];
  }
  node_mutex.clear();
  VLOG(0) << "end to process fennel bx egde shard";
}

void GraphTable::fennel_graph_feature_partition() {
  VLOG(0) << "start to process fennel feature shard";
  std::vector<std::vector<GraphShard *>> tmp_feature_shards;
  tmp_feature_shards.resize(id_to_feature.size());
  for (size_t k = 0; k < tmp_feature_shards.size(); k++) {
    for (size_t i = 0; i < shard_num_per_server; i++) {
      ...
      tasks.push_back(load_node_edge_task_pool->enqueue(
          [&, node_idx, i, this]() -> std::pair<size_t, size_t> {
            ...
            for (auto node : shards) {
              ...
              bool find_this_egde_node = false;
              int find_index = -1;
              for (int idx = 0; idx < node_num_; idx++) {
                if (egde_node_ids_[idx].find(id) != egde_node_ids_[idx].end()) {
                  find_index = idx;
                  break;
                }
              }
              if (find_index == node_id_) {
                find_this_egde_node = true;
              } else if (find_index == -1) {
                singe_num++;
              }

              // 孤立点 和 在egde_node_ids_[node_id]的点，都保留
              if (find_this_egde_node ||
                  (find_index == -1 && is_key_for_self_rank(id))) {
                num++;
                tmp_feature_shards[node_idx][i]->add_graph_node(node);
              }
            }
            return {singe_num, num};
          }));
    }
  }
  size_t single_keys = 0;
  size_t keys_all = 0;
  for (size_t i = 0; i < tasks.size(); ++i) {
    auto res = tasks[i].get();
    single_keys += res.first;
    keys_all += res.second;
  }
  // 替换原来的shards
  VLOG(0) << "begin to replace feature shard";
  // clear_feature_shard();
  feature_shards = std::move(tmp_feature_shards);

  VLOG(0) << "end to process fennel feature shard, single_keys:" << single_keys
          << " all_keys:" << keys_all;
}

// void GraphTable::hard_graph_feature_partition() {
//   // 取边表和节点表的并集作为最终结果
//   std::vector<std::mutex *> fea_mutex(feature_shards.size());
//   for (size_t i = 0; i < feature_shards.size(); i++) {
//     fea_mutex[i] = new std::mutex();
//   }

//   std::vector<std::future<size_t>> tasks;
//   for (size_t i = 0; i < all_edge_keys_.size(); i++) {
//     tasks.push_back(load_node_edge_task_pool->enqueue(
//         [&, i, this]() -> size_t {
//       size_t new_add_num = 0;
//       for (size_t j = 0; j < all_edge_keys_[i].size(); j++) {
//         int id = all_edge_keys_[i][j];
//         Node *node = find_node(GraphTableType::FEATURE_TABLE, id);
//         if (node == nullptr) {
//           auto edge_type_list =
//                 paddle::string::split_string<std::string>(id_to_edge[i],
//                 "2");
//           if (feature_to_id.find(edge_type_list[0]) == feature_to_id.end()) {
//             VLOG(0) << "failed to find edge_type:" << edge_type_list[0];
//             continue;
//           }
//           int node_type_id = feature_to_id[edge_type_list[0]];
//           fea_mutex[node_type_id]->lock();
//           feature_shards[node_type_id][id % shard_num]->add_feature_node(id,
//           false, 0); fea_mutex[node_type_id]->unlock(); new_add_num++;
//         }
//       }
//       return new_add_num;
//     }));
...
//   size_t all_new_add_num = 0;
//   for (size_t i = 0; i < tasks.size(); ++i) {
//     all_new_add_num += tasks[i].get();

//   }
//   VLOG(0) << "hard_graph_feature_partition all_new_add_num:" <<
//   all_new_add_num;
// }
...
void GraphTable::hard_graph_feature_partition() {
  ...
  for (auto &it : this->feature_to_id) {
    ...
    for (size_t i = 0; i < shard_num_per_server; i++) {
      tasks.push_back(
          load_node_edge_task_pool->enqueue([&, node_idx, i, this]() -> size_t {
            size_t num = 0;
            auto &shards = feature_shards[node_idx][i]->get_bucket();
            for (auto node : shards) {
              uint64_t id = node->get_id();

              bool find_egde_node = false;
              bool find_this_egde_node = false;
              for (int idx = 0; idx < node_num_; idx++) {
                if (egde_node_ids_[idx].find(id) != egde_node_ids_[idx].end()) {
                  find_egde_node = true;
              ...
              if (egde_node_ids_[node_id_].find(id) !=
                  egde_node_ids_[node_id_].end()) {
                find_this_egde_node = true;
              }

              // 孤立点 和 在egde_node_ids_[node_id]的点，都保留
              if (find_this_egde_node ||
                  (!find_egde_node && is_key_for_self_rank(id))) {
                num++;
                tmp_feature_shards[node_idx][i]->add_graph_node(node);
              }
            }
  ...
  VLOG(0) << "end to process hard2 feature shard, keys_all:" << keys_all;
}
...
int GraphTable::partition_for_rank(const uint64_t &id) {
#if defined(PADDLE_WITH_HETERPS) && defined(PADDLE_WITH_GLOO)
  thread_local auto ps_wrapper = paddle::framework::PSGPUWrapper::GetInstance();
  return ps_wrapper->PartitionKeyForRank(id);
#else
  return 0;
#endif
}
...
void GraphTable::build_graph_type_keys() {
  ...
  graph_type_keys_set_.clear();
  graph_type_keys_set_.resize(this->feature_to_id.size());
  ...
  for (auto &it : this->feature_to_id) {
    ...
    graph_type_keys_set_[cnt].insert(keys[0].begin(), keys[0].end());
