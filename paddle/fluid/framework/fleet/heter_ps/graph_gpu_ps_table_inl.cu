...
__global__ void compress_sample_result_with_type_info(
    ...
    int* dst_key_index,
    int64_t* dst_val,
    int64_t* dst_node_type) {
  const size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < n && actual_sample_size[i] > 0) {
    int base = actual_sample_size_sum[i];
    int edge_idx = 0;
    int len = 0;
    for (; i * sample_size >= len + global_edge_type_result_len[edge_idx];
         edge_idx++) {
...
__global__ void rearrange_sample_result_with_type_info(
  ...
  if (i < n) {
    ...
    for (int j = 0; j < dist_actual_sample_size[ind]; j++) {
      dist_key_index[dist_key_start + j] = idx[node_id];
      ...
      if (node_info->neighbor_size == 0) {
        *actual_size = 0;
        ...
        uint64_t* sample_array =
            sample_array_base + result_start[edge_idx] + offset * sample_size;
  ...
  __global__ void neighbor_sample_kernel(GpuPsCommGraph graph,
    ...
    while (i < last_idx) {
      ...
      if (neighbor_len <= sample_len) {
        ...
        __syncwarp();
        for (int j = sample_len + threadIdx.x; j < neighbor_len;
             j += WARP_SIZE) {
  ...
  */
  ...
  void GpuPsGraphTable::display_sample_res(
      void* key, void* val, int len, int sample_len) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      int ac_size =
          *(int*)(val_buffer + i * sizeof(int) + len * sizeof(int64_t));
  ...
  void GpuPsGraphTable::move_result_to_source_gpu(int start_index,
    ...
    for (int i = 0; i < gpu_num; i++) {
      ...
      CUDA_CHECK(cudaMemcpyAsync(
          ...
          node.out_stream));
      CUDA_CHECK(cudaMemcpyAsync(
          reinterpret_cast<char*>(actual_sample_size + h_left[i]),
          node.val_storage + sizeof(int64_t) * shard_len[i],
  ...
  */
  ...
  __global__ void node_query_example(
      GpuPsCommGraph graph, int start, int size, uint64_t* res) {
    const size_t i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < size) {
  ...
  */
  void GpuPsGraphTable::build_graph_fea_on_single_gpu(
      const GpuPsCommGraphFea& g, int gpu_id) {
  ...
  void GpuPsGraphTable::split_node_with_types_to_shard(uint64_t * d_keys,
    ...
    int* d_shard_index_tmp_ptr =
        reinterpret_cast<int*>(d_shard_index_tmp->ptr());
    ...
    calc_shard_index_with_node_type_kernel<<<grid_size,
                                             block_size_,
                                             0,
                                             stream>>>(d_keys,
                                                       node_types,
  ...
  gpu i saves the ith graph from cpu_graph_list
  */
  void GpuPsGraphTable::build_graph_on_single_gpu(
      const GpuPsCommGraph& g, int i, int idx) {
    ...
    int table_offset = get_table_offset(i, GraphTableType::EDGE_TABLE, idx);
    size_t capacity =
        std::max((uint64_t)1, (uint64_t)g.node_size) / load_factor_;
    tables_[table_offset] = new Table(capacity);
    if (g.node_size > 0) {
  ...
  void GpuPsGraphTable::build_graph_fea_from_cpu(
    ...
    PADDLE_ENFORCE_EQ(cpu_graph_fea_list.size(),
  ...
  void GpuPsGraphTable::build_graph_from_cpu(
    ...
    VLOG(0) << "in build_graph_from_cpu cpu_graph_list size = "
            << cpu_graph_list.size();
    PADDLE_ENFORCE_EQ(cpu_graph_list.size(),
                      resource_->total_device(),
                      platform::errors::InvalidArgument(
                          "the cpu node list size doesn't match "
                          "the number of gpu on your machine."));
    ...
    for (int i = 0; i < cpu_graph_list.size(); i++) {
      ...
      gpu_graph_list_[offset] = GpuPsCommGraph();
      tables_[table_offset] = new Table(
          std::max((uint64_t)1, (uint64_t)cpu_graph_list[i].node_size) /
          load_factor_);
  ...
  NeighborSampleResult GpuPsGraphTable::graph_neighbor_sample(
      int gpu_id, uint64_t* key, int sample_size, int len) {
  ...
  NeighborSampleResult GpuPsGraphTable::graph_neighbor_sample_v2(
    ...
    uint64_t* d_shard_keys_ptr =
        reinterpret_cast<uint64_t*>(d_shard_keys->ptr());
    auto d_shard_vals =
        memory::Alloc(place, sample_size * len * sizeof(uint64_t));
    uint64_t* d_shard_vals_ptr =
        reinterpret_cast<uint64_t*>(d_shard_vals->ptr());
    ...
    for (int i = 0; i < total_gpu; ++i) {
      ...
      neighbor_sample_kernel2<<<grid_size_,
                                block_size_,
                                0,
                                resource_->remote_stream(i, gpu_id)>>>(
          graph,
    ...
    if (cpu_query_switch) {
      ...
      if (number_on_cpu > 0) {
        ...
        uint64_t* gpu_buffers_ptr =
            thrust::raw_pointer_cast(gpu_buffers.data());
        ...
        get_actual_gpu_ac<<<grid_size2, block_size_, 0, stream>>>(
            gpu_ac_ptr, number_on_cpu);
    ...
    if (compress) {
      int total_sample_size =
          thrust::reduce(thrust::device_pointer_cast(actual_sample_size),
  ...
  void GpuPsGraphTable::set_edge_in_type(vector<int> & edge_in_type) {
    edge_in_type_ = edge_in_type;
  }
  void GpuPsGraphTable::set_edge_out_type(vector<int> & edge_out_type) {
  ...
  GpuPsGraphTable::sample_neighbor_with_node_type(
    ...
    // result.initialize(sample_size, len, edge_type_len,
    // resource_->dev_id(gpu_id)); if (len == 0) {
    ...
    CUDA_CHECK(cudaMemsetAsync(
        d_right_ptr, -1, total_partition * sizeof(int), stream));
    ...
    /*
    auto d_shard_vals =
        memory::Alloc(place, sample_size * len * edge_type_len *
    sizeof(uint64_t)); uint64_t* d_shard_vals_ptr =
    reinterpret_cast<uint64_t*>(d_shard_vals->ptr()); auto
    d_shard_actual_sample_size = memory::Alloc(place, len * edge_type_len *
    sizeof(int)); int* d_shard_actual_sample_size_ptr =
        reinterpret_cast<int*>(d_shard_actual_sample_size->ptr());
        */
    ...
    int edge_type_key_start[total_device][edge_type_len],
        edge_type_key_len[total_device][edge_type_len];
    memset(edge_type_key_start, 0, sizeof(edge_type_key_start));
    memset(edge_type_key_len, 0, sizeof(edge_type_key_len));
    int edge_type_result_start[total_device][edge_type_len],
        edge_type_result_len[total_device][edge_type_len];
    memset(edge_type_result_start, 0, sizeof(edge_type_result_start));
    memset(edge_type_result_len, 0, sizeof(edge_type_result_len));
    int global_edge_type_key_start[total_device][edge_type_len];
    int global_edge_type_result_start[total_device][edge_type_len];
    memset(global_edge_type_key_start, 0, sizeof(edge_type_result_start));
    memset(global_edge_type_result_start, 0, sizeof(edge_type_result_len));
    for (int i = 0; i < edge_type_len; i++) {
      ...
      for (int j = 0; j < total_gpu; j++) {
        ...
        if (h_left_with_node_type[part_id] != -1) {
          ...
          edge_type_key_start[j][i] =
              h_left_with_node_type[part_id] - h_left[j];
        }
      }
    }
    int total_result_len[total_gpu], length_sum = 0,
                                     total_result_len_prefix[edge_type_len];
    for (int i = 0; i < total_gpu; i++) {
      ...
      length_sum += total_result_len[i];
    }
    int cursor = 0;
    for (int j = 0; j < edge_type_key_len; j++) {
      for (int i = 0; i < tota_gpu; i++) {
        global_edge_type_result_start[i][j] = cursor;
        cursor += edge_type_result_len[i][j];
      }
      total_result_len_prefix[j] = cursor;
    }
    auto d_edge_type_key_len =
        memory::Alloc(place, tota_gpu * edge_type_key_len * sizeof(int));
    ...
    auto d_global_edge_type_result_len =
        memory::Alloc(place, edge_type_key_len * sizeof(int));
    auto d_edge_out_type =
        memory::Alloc(place, edge_type_key_len * sizeof(int));
    cudaMemcpyAsync(reinterpret_cast<int*>(d_edge_type_key_len->ptr()),
                    (int*)edge_type_key_len,
                    tota_gpu * edge_type_key_len * sizeof(int),
                    cudaMemcpyHostToDevice,
                    stream);
    ...
    size_t val_len[tota_gpu];
    for (int i = 0; i < total_gpu; ++i) {
      int shard_len = h_left[i] == -1 ? 0 : h_right[i] - h_left[i] + 1;
      if (shard_len == 0) {
        continue;
      }
      val_len[i] = total_result_len[i] / sample_size * sizeof(uint64_t) +
                   total_result_len[i] * sizeof(uint64_t) +  // sample
                   total_result_len[i] / sample_size *
                       sizeof(int);  // actual_sample size
      // no align
    ...
    for (int i = 0; i < total_gpu; ++i) {
      ...
      for (int idx = 0; idx < edge_type_len; idx++) {
        ...
        int table_offset = get_table_offset(i, GraphTableType::EDGE_TABLE, idx);
      tables_[table_offset]->get(reinterpret_cast<uint64_t*>(node.key_storage) + edge_type_key_start[i][idx],
                                 reinterpret_cast<uint64_t*>(node_info_base) + edge_type_result_start[i][idx]/sample_size),
      ...
      GpuPsCommGraph* d_commgraph_ptr =
          reinterpret_cast<GpuPsCommGraph*>(d_commgraph_mem->ptr());
      int* actual_size_base =
          (int*)(node_info_base + total_result_len[i] / sample_size +
                 total_result_len[i]);
      ...
      neighbor_sample_kernel_with_type_info<<<grid_size_,
          ...
          key_size,
          edge_type_len,
          reinterpret_cast<int*>(d_edge_type_key_start->ptr()) reinterpret_cast<
              int*>(
              d_edge_type_key_len
                  ->ptr()) reinterpret_cast<int*>(d_edge_type_result_start
                                                      ->ptr()) reinterpret_cast<int*>(d_edge_type_result_len
                                                                                          ->ptr()));
    }
    ...
    auto d_local_actual_sample_size =
        memory::Alloc(place, cursor / sample_size * sizeof(int));
    auto d_edge_split_num =
        memory::Alloc(place, edge_type_key_len * sizeof(int));
    int* d_edge_split_num_ptr = reinterpret_cast<int*>(d_edge_split_num);
    ...
    for (int i = 0; i < gpu_num; i++) {
      ...
      auto& node = path_[start_index][i].nodes_.front();
      int key_size = total_result_len[i] / sample_size;
      int grid_size = (key_size / sample_size - 1) / block_size_ + 1;
      rearrange_sample_result_with_type_info<<<grid_size,
          ...
          edge_type_len,
          key + h_left[i],
          (uint64_t*)node.val_storage + key_size,
          idx + h_left[i],
          reinterpret_cast<int*>(d_edge_type_key_start->ptr()) +
              i * edge_type_len,
          reinterpret_cast<int*>(d_edge_type_key_len->ptr()) +
              i * edge_type_len,
          reinterpret_cast<int*>(d_edge_type_result_start->ptr()) +
              i * edge_type_len,
          reinterpret_cast<int*>(d_global_edge_type_result_start->ptr()) +
              i * edge_type_len,
          reinterpret_cast<int*>(d_local_key_index->ptr()),
          reinterpret_cast<uint64_t*>(d_local_val->ptr()),
          (int*)(node.val_storage +
    ...
    auto cumsum_actual_sample_size =
        memory::Alloc(place_, (cursor / sample_size + 1) * sizeof(int));
  ...
  for (int i = 0; i < edge_type_len; i++) {
      cudaMemcpyAsync(
          ...
          cumsum_actual_sample_size_ptr + total_result_len_prefix[i];
  ...
  auto final_key_index = memory::AllocShared(place, all_sample_size * sizeof(int));
  int final_key_index_ptr = reinterpret_cast<int* >(final_key_index->ptr());
  auto neighbor_index = 
      memory::AllocShared(place, all_sample_size * sizeof(int));
  int * neighbor_index_ptr = reinterpret_cast<int64_t* >(neighbor_index->ptr());
  int grid_size_compress = (cursor/sample_size - 1) / block_size_ + 1;
  auto d_node_type_tmp = memory::Alloc(place, (all_sample_size) * sizeof(int64_t));
  compress_sample_result_with_type_info<<<grid_size_compress, block_size_,0,stream>>>(cursor/sample_size,
  sample_size, edge_type_len,
  reinterpret_cast<int*>(d_local_actual_sample_size->ptr()),
  ...
  reinterpret_cast<int64_t *> (d_node_type_tmp_ptr->ptr());
  ...
  NeighborSampleResultV2 GpuPsGraphTable::graph_neighbor_sample_all_edge_type(
    ...
    NeighborSampleResultV2 result;
    result.initialize(
        sample_size, len, edge_type_len, resource_->dev_id(gpu_id));
    if (len == 0) {
      return result;
    }
    ...
    for (int i = 0; i < total_gpu; ++i) {
      ...
      GpuPsCommGraph* d_commgraph_ptr =
          reinterpret_cast<GpuPsCommGraph*>(d_commgraph_mem->ptr());
      int* actual_size_base =
          (int*)(node_info_base + shard_len * edge_type_len);
      uint64_t* sample_array_base =
      ...
      neighbor_sample_kernel3<<<grid_size_,
                                block_size_,
                                0,
                                resource_->remote_stream(i, gpu_id)>>>(
          d_commgraph_ptr,
  ...
  NodeQueryResult GpuPsGraphTable::query_node_list(
      int gpu_id, int idx, int start, int query_size) {
    ...
    int x2, y2;
    int len =
        range_check(start, start + query_size, 0, graph.node_size, x2, y2);
  ...
  int GpuPsGraphTable::get_feature_of_nodes(int gpu_id,
    ...
    for (int i = 0; i < total_gpu; ++i) {
      ...
      GpuPsFeaInfo* val_array =
          reinterpret_cast<GpuPsFeaInfo*>(node.val_storage);
