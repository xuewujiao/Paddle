...
__global__ void GraphOneIdKernel(uint64_t *id_tensor,
                                   int len) {
  size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
  uint64_t onekey = 1;
  if (idx < len) {
    int dst = idx * 2;
    id_tensor[dst] = onekey;
    id_tensor[dst + 1] = onekey;
  }
}
  ...
  if (!conf_.gpu_graph_training) {
    ...
    if (!conf_.sage_mode) {
            ...
            if (ins_buf_pair_len_[tensor_pair_idx] == 0) {
              ...
              if (conf_.is_multi_node) {
                ...
                if (total_row_[tensor_pair_idx] != 0) {
                  ...
                  //add fill 1 ins kernel
                  GraphOneIdKernel<<<GET_BLOCKS(fill_one_num), CUDA_NUM_THREADS, 0, train_stream_>>>(
                      reinterpret_cast<uint64_t *>(d_ins_buf_[tensor_pair_idx]->ptr()),
                      fill_one_num);
                  VLOG(0) << "end set one ins";
                  break;
                }
              } else {
                return 0;
      ...
      if (conf_.is_multi_node && total_row_[0] == 0) {
        total_instance = fill_one_num;
        ins_buf_pair_len_[0] = fill_one_num;
        VLOG(2) << "gpu id: " << conf_.gpuid << "set total ins num: " << fill_one_num;
      }
