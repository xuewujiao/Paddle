...
__global__ void fill_dvalues_with_edge_type(int* d_shard_degree,
                                            int* node_degree,
                                            int* idx,
                                            int len,
                                            int mod) {
  ...
  if (i < len) {
    int a = i % mod, b = i - i % mod;
    node_degree[b + idx[a]] = d_shard_degree[i];
  }
}
...
void GpuPsGraphTable::get_node_degree(
    int gpu_id, int idx, uint64_t* key, int len, 
    std::shared_ptr<phi::Allocation> node_degree) {
  
  int* node_degree_ptr = 
      reinterpret_cast<int *>(node_degree->ptr()) + idx * len;
  ...
  auto d_shard_degree = memory::Alloc(place, len * sizeof(int));
