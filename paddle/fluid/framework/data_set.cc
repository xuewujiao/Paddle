...
void SlotRecordDataset::DynamicAdjustBatchNum() {
  ...
  if (gpu_graph_mode_) {
    ...
    for (size_t i = 0; i < readers_.size(); i++) {
      ...
      if (batch_num > thread_avg_batch_num) {
        thread_avg_batch_num = batch_num;
      }
      VLOG(3) << "ins num:" << ins_num << ", batch size:"
              << batch_size << ", batch_num:" << thread_avg_batch_num;
    }
#ifdef PADDLE_WITH_GLOO
    auto gloo_wrapper = paddle::framework::GlooWrapper::GetInstance();
    if (gloo_wrapper->Size() > 1) {
    ...
    auto thread_max_batch_num_vec =
                 gloo_wrapper->AllReduce(thread_avg_batch_num_vec, "max");
    int thread_max_batch_num = thread_max_batch_num_vec[0];
    VLOG(3) << "max batch_num:" << thread_max_batch_num << ", avg batch num:" << thread_avg_batch_num;
    thread_avg_batch_num = thread_max_batch_num;
    for (size_t i = 0; i < readers_.size(); i++) {
      readers_[i]->SetNewBatchsize(thread_avg_batch_num);
    }
#endif
  }
#endif
