...
__global__ void weight_sample_large_kernel(GpuPsCommGraph graph,
  ...
  if (neighbor_len <= sample_len) {  // directly copy
    ...
    RandomNumGen rng(gidx, random_seed);  // get weight thredshold
    for (int j = threadIdx.x; j < neighbor_len; j += BLOCK_SIZE) {
  ...
  for (int i = 0; i < total_gpu; ++i) {
    ...
    if (!weighted) {
      ...
      if (need_neighbor_count) {
        ...
        get_actual_size_and_neighbor_count<true><<<grid_size, 128, 0, cur_stream>>>(
            node_info_list, actual_size_array, neighbor_count_ptr, sample_size, shard_len,
            default_value);
      } else {
        int grid_size = (shard_len + 127) / 128;
...
NeighborSampleResultV2 GpuPsGraphTable::graph_neighbor_sample_all_edge_type(
  ...
  int* actual_sample_size = result.actual_sample_size;
  uint64_t* val = result.val;
  float* edge_weight = result.edge_weight;
  int total_gpu = resource_->total_device();
  ...
  float* d_shard_edge_weight_ptr = nullptr;
  if (return_weight) {
    auto d_shard_edge_weight =
        memory::Alloc(place,
                      sample_size * len * edge_type_len * sizeof(float),
                      phi::Stream(reinterpret_cast<phi::StreamId>(stream)));
    d_shard_edge_weight_ptr =
        reinterpret_cast<float*>(d_shard_edge_weight->ptr());
  }
    ...
    if (!return_weight) {
      ...
      create_storage(
          ...
          shard_len * sizeof(uint64_t) * edge_type_len +  // key
              ...
              (((shard_len * edge_type_len) * (1 + sample_size)) % 2) * sizeof(int));  // align, sizeof(int) == sizeof(float)
    }
  }
  walk_to_dest(gpu_id,
               total_gpu,
        ...
        if (need_neighbor_count) {
          get_actual_size_and_neighbor_count<true><<<
              grid_size, 128, 0, cur_stream>>>(node_info_list, actual_size_array,
                                               neighbor_count_ptr, sample_size, shard_len,
                                               default_value);
        } else {
