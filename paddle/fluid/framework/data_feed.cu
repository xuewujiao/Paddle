...
__global__ void GraphFillIdKernel(uint64_t *id_tensor,
                                  ...
                                  int32_t *edge_labels_conf,
                                  int node_type_num) {
  __shared__ uint64_t local_key[CUDA_NUM_THREADS * 2];
  __shared__ uint8_t local_edge_label[CUDA_NUM_THREADS];
  ...
  if (idx < len) {
    ...
    if ((src + step) >= last_row && (src + step) < next_row &&  walk[src] != 0 && walk[src + step] != 0) {
      ...
      if (!need_filter) {
        ...
        if (edge_labels_conf != NULL) {
          int edge_labels_conf_index = walk_ntype[src] * node_type_num + walk_ntype[src + step];
          local_edge_label[dst] = edge_labels_conf[edge_labels_conf_index];
        }
      }
    }
  }
  ...
  if (threadIdx.x < local_num) {
    ...
    if (edge_labels_conf != NULL) {
      edge_label_tensor[global_num + threadIdx.x] = local_edge_label[threadIdx.x];
    }
  }
}
...
int GraphDataGenerator::FillIdShowClkTensor(int total_instance,
  ...
  if (gpu_graph_training) {
    ...
    if (enable_edge_label_) {
      edge_label_ptr_ = feed_vec_[3]->mutable_data<int32_t>({total_instance / 2}, this->place_);
      int32_t *edge_label_buf = reinterpret_cast<int32_t *>(d_edge_label_buf_->ptr());
      int32_t *edge_label_cursor = edge_label_buf + ins_buf_pair_len_ - total_instance / 2;
      cudaMemcpyAsync(edge_label_ptr_, edge_label_cursor, sizeof(int32_t) * total_instance / 2,
...
int GraphDataGenerator::FillGraphIdShowClkTensor(int uniq_instance,
  ...
  int index_offset = 0;
  if (enable_edge_label_) {
    edge_label_ptr_ = feed_vec_[3]->mutable_data<int32_t>({uniq_instance / 2}, this->place_);
  }
  if (!return_weight_) {
...
int GraphDataGenerator::MakeInsPair(cudaStream_t stream) {
  ...
  int32_t *edge_label_buf = NULL;
  int32_t *edge_labels_conf = NULL;
  if (enable_edge_label_) {
    edge_label_buf = reinterpret_cast<int32_t *>(d_edge_label_buf_->ptr());
    edge_labels_conf = reinterpret_cast<int32_t *>(d_edge_labels_conf_->ptr());
  }
  ...
  GraphFillIdKernel<<<GET_BLOCKS(len), CUDA_NUM_THREADS, 0, stream>>>(
      ...
      edge_label_buf + ins_buf_pair_len_,
      d_pair_num,
      walk,
      walk_ntype,
...
void GraphDataGenerator::AllocResource(
  ...
  d_pair_num_ = memory::AllocShared(place_, sizeof(int),
          phi::Stream(reinterpret_cast<phi::StreamId>(sample_stream_)));
  enable_edge_label_ = gpu_graph_training_ && gpu_graph_ptr->edge_labels_conf_.size() > 0;
  if (enable_edge_label_) {
    node_type_num_ = gpu_graph_ptr->id_to_feature.size();
    d_edge_label_buf_ = memory::AllocShared(place_, (batch_size_ * 2) * sizeof(int32_t),
            phi::Stream(reinterpret_cast<phi::StreamId>(sample_stream_)));
    d_edge_labels_conf_ = memory::AllocShared(place_,
            node_type_num_ * node_type_num_ * sizeof(int32_t),
            phi::Stream(reinterpret_cast<phi::StreamId>(sample_stream_)));
    CUDA_CHECK(cudaMemcpyAsync(d_edge_labels_conf_->ptr(),
                               gpu_graph_ptr->edge_labels_conf_.data(),
                               node_type_num_ * node_type_num_ * sizeof(int32_t),
  ...
  need_walk_ntype_ = excluded_train_pair_len_ > 0 || enable_edge_label_;
  if (need_walk_ntype_) {
...
void GraphDataGenerator::SetConfig(
  ...
  gpu_graph_ptr->init_conf(
      first_node_type, meta_path, graph_config.excluded_train_pair(), graph_config.edge_labels());
